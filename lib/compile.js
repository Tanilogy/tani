...
function compileQueries(tribe, locality) {
  ...
  Object.keys(util.concepts).forEach(function (concept) {
    ...
    if (fs.existsSync(dialectsConstructDir)) {
      ...
      compileEntries(dialectsConstructDir, tribe, locality, concept, script)
...
function compileEntries(dialectsConstructDir, tribe, locality, concept, script, subDir) {
  ...
  files.forEach(function (file) {
    ...
    if (util.ignore.indexOf(file) === -1) {
      ...
      if (stat.isDirectory()) {
        return compileEntries(currentFilePath, tribe, locality, concept, script, file)
      }
      else {

        var dialectFilePath = localityDialectsDir + '/' + concept + '/' + (subDir ? subDir + '/' : '') + file
        ...
        if (dialectEntry.length) {
          ...
          var fileName = subDir + '-' + file.replace('txt', 'acf')
          ...
          var globalID = subDir.toUpperCase() + '-' + file.replace('.txt', '').toUpperCase()
          var queryFilePath = localityQueriesDir + '/' + concept + '/entries/' + fileName
...
function initCompileConcepts() {
  ...
  Object.keys(util.concepts).forEach(function (conceptType) {
    ...
    console.log(currentConceptsDir, conceptType)
