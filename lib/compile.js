    ...
    if (util.ignore.indexOf(file) === -1) {
      ...
      if (stat.isDirectory()) {
        ...
        if (dialectEntry.length) {
          ...
          var includeConceptString = 'include ' + concept + '/index.acf\n'
          ...
          if (parsed) {
            ...
            var includeEntriesString = 'include entries/' + globalID.toLowerCase() + '.acf\n'
...
function initCompileConceptsForDialect() {
  return
  ...
  Object.keys(util.concepts).forEach(function (conceptType) {
    ...
    compileConceptsForDialect(currentConceptsDir, conceptType)
  })
...
function parseDialectFile(dialectFilePath, globalID, concept, script, localityPath) {
  ...
  if (root) {
    ...
    var createTemplateFile = templatesDir + '/create.txt'
