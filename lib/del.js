  ...
  if (temp.length > 0) {
    ...
    if (temp.length === 1) {
      ...
      if (fs.existsSync(tribeDir)) {
        ...
        var queryDir = process.cwd() + '/system/queries/' + tribe
        fs.removeSync(dialectDir)
        fs.removeSync(queryDir)

        var tribeIndex = process.cwd() + '/system/queries/index.acf'
...
function removeEntry(tribe, locality) {
  ...
  var localityQueriesDir = process.cwd() + '/system/queries/' + localityPath
