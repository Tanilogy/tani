...
module.exports = function (argv) {
  ...
  if (argv._.length > 1) {
    ...
    if (argv._.indexOf('from') === 2 && argv._.length === 4) {
      ...
      if (fs.existsSync(fromQueriesDir)) {
        ...
        var localityIndexFile = path.join(toQueriesDir, 'index.acf')
        var localityIndexFileContent = fs.readFileSync(localityIndexFile).toString()
        var fromLocality = from.split('/')[1].capitalize()
        var toLocality = to.split('/')[1].capitalize()
        localityIndexFileContent = localityIndexFileContent.replace(fromLocality, toLocality)
        fs.writeFileSync(localityIndexFile, localityIndexFileContent)
      ...
      Object.keys(util.constructs).forEach(function (construct) {
        ...
        var constructlocalityPath = localityQueriesDir + '/' + construct
        var constructEntries = constructlocalityPath + '/entries'
        var constructIndex = constructlocalityPath + '/index.acf'

        fs.mkdirsSync(constructlocalityPath)
