    ...
    if (argv._.indexOf('from') === 2 && argv._.length === 4) {
      ...
      Object.keys(util.constructs).forEach(function (construct) {
        ...
        if (fs.existsSync(constructConceptDir)) {
          ...
          initDialectFile(constructConceptDir, localityPath, construct)
...
function initDialectFile(constructConceptDir, localityPath, construct, subDir) {
  ...
  files.forEach(function (file) {
    ...
    if (stat.isDirectory()) {
      return initDialectFile(filePath, localityPath, construct, file)
    }
    else {
