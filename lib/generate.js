...
module.exports = function () {
  ...
  if (fs.existsSync(conceptsDir)) {
    ...
    fs.readdirSync(conceptsDir).forEach(function (construct) {
      ...
      if (util.ignore.indexOf(construct) < 0) {
        ...
        var definitionConstructDir = path.join(process.cwd(), 'concepts', construct)
        fs.readdirSync(definitionConstructDir).forEach(function (definitionFile) {

          if (util.ignore.indexOf(construct) < 0) {

            var definitionFilePath = path.join(process.cwd(), 'concepts', construct, definitionFile)
