...
var util = module.exports = {
  ...
  moveFileIndex: function(targetDir, startingIndex, currentHighestIndex) {

    // increment not required
    if (+startingIndex === currentHighestIndex) return

    // if id is to be at a lower value
    if (startingIndex < currentHighestIndex) {
      var currentName = targetDir + '/' + currentHighestIndex + '.txt'
      var tempName = targetDir + '/' + (currentHighestIndex + 1) + '.txt'
      fs.renameSync(currentName, tempName)

      var indexCounter = currentHighestIndex - 1
      while (indexCounter >= startingIndex) {
      ...
      var indexedName = targetDir + '/' + startingIndex + '.txt'
